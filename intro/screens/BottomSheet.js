...
export default function BottomSheet() {
  ...
  const startPosition = height - 100; // posición inicial
  const animatedValue = useRef(new Animated.Value(startPosition)).current; // animación de movimiento

  // Limites
  const topLimit = height - bottomSheetHeight; // abierto
  const bottomLimit = startPosition; // cerrado

  // PanResponder (traductor dedo-codigo)
